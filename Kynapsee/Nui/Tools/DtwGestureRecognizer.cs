...
namespace Kynapsee.Nui.Tools
{
    ...
    public class DtwGestureRecognizer
    {
        ...
        /// </summary>
        public Dictionary<Gesture, Dictionary<JointID, List<Vector4>>> _recordedGestures;
        ...
        public DtwGestureRecognizer(int dim, int noJoints, double recogThreshold, double posThreshold, int minLen)
        {
            ...
            _recordedGestures = new Dictionary<Gesture, Dictionary<JointID, List<Vector4>>>();
        ...
        public Gesture Recognize(Dictionary<JointID, List<Vector4>> inputSequence)
        {

            double minDist = double.PositiveInfinity;
            Gesture classification = null;

            foreach (Gesture gesture in _recordedGestures.Keys)
            {
                foreach (JointID joint in _recordedGestures[gesture].Keys)
                {

                    int lastInputPosition = inputSequence[joint].Count - 1;
                    int lastRecordedPosition = _recordedGestures[gesture][joint].Count - 1;

                    if (CalculateSnapshotPositionDistance(inputSequence, lastInputPosition, _recordedGestures[gesture], lastRecordedPosition) < _positionThreshold)
                    {
                        //We've met the positionThreshold requirement, now perform DTW recognition
                        double d = Dtw(inputSequence, _recordedGestures[gesture]) / _recordedGestures[gesture][joint].Count;
                        if (d < minDist)
                        {
            ...
            return (minDist < _recognitionThreshold ? classification : null) /*+ " " +minDist.ToString()*/;
        }
